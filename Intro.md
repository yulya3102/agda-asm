...
получается как-то не очень. Есть, к примеру, VeLLVM [@vellvm], который формализует
язык LLVM и делает доказанно корректные оптимизации в нем. Еще есть
CompCert [@compcert], который представляет собой компилятор, делающий внутри себя
...
чуваки в 2015 делали стресс-тесты для линкеров [@ltostress], и находили просто уйму
...
области есть Bedrock [@bedrock], который можно назвать чем-то типа макро-языка для
ассемблера, формализованного на Coq[@coq]. Они пытались что-то делать с
линковкой [@bedrocklinkers], но получилось не очень.

Существует хорошая формализация ассемблера — TAL [@tal] от Моррисетта и компании,
и много его расширений, покрывающих, например, почти настоящий язык
ассемблера x86 (TALx86) [@talx86], работу со стеком (STAL) [@stal] и даже объектные файлы
(MTAL) [@mtal]. Кстати говоря, в той же работе, что описывает MTAL, пытались что-то
...
статье используется формализация на агде [@agda], близкая к смеси TALx86, STAL и
