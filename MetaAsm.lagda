...
например, копирование значения из ячейки памяти в регистр. Такими типами
являются целые числа (соответствующие конструктору \C{int}) и указатели
(описываемые конструктором \C{\_*}). \F{Type} описывает типы произвольного
размера, которые включают в себя типы размером с регистр (через
конструктор \C{atom}) и блоки кода (с помощью конструктора \C{code}).
}{
