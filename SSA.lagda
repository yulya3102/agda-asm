...
на какой контекст он рассчитывает и как он его меняет. Это может быть
полезным, например, при определении функции, загружающей произвольный
блок кода из памяти.
...
Результатом исполнения блока является не только измененный контекст, но и
измененное состояние регистров. Но в данной реализации рассматриваются
только переходы между блоками и никак не учитывается состояние регистров,
...
исполнения, если для обоих блоков указана последовательность исполнения,
