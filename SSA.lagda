...
память кода, удобно ввести понятие _базового блока кода_ — последовательности
...
*   вызов соответствующего блока кода, находящегося в памяти по данному
    адресу;
...
Непрямой `jmp` не меняет стек и передает управление на блок, загруженный
...
`jmp` не меняет стек и передает управление так же, как это делает вызов
функции:
...
Блок кода эквивалентен непрямому `jmp`, если по указанному адресу находится
