...
генерируются на каждый блок кода. Функция \F{pltize} показывает, как
...
он делает \C{jmp[\_]} на тот адрес, который записан в GOT. Динамический
...
блока PLT будет приводить к исполнению самой функции. \C{dempty} в последнем
