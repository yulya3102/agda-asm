...
В типе функции \F{plt-stub} последним параметром типа возвращаемого блока
является некоторое описание того, какие части состояния исполнителя меняет
этот блок. В данном случае это \C{dempty}: пустое изменение состояния,
потому что приведенный блок PLT ничего не меняет. И это абсолютно
правильно, потому что в противном случае вызов функции через ее блок PLT не
был бы прозрачным и менял бы семантику программы.
