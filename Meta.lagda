...
как параметры. Набор этих определений будем называть _метаассемблером_.
...
*   Модуль `ExecBlk`, принимающий параметрами типы инструкций и управляющих
    ...
    состояние исполнителя. Определяют функцию, определяющую, как блок
...
В этой версии память больше не считается неизменной, поэтому тип состояния
исполнителя должен описывать не только регистры, но и память.
...
регистров отдельно от всего состояния исполнителя, удобно определить
функцию для применения изменений сразу ко всему состоянию исполнителя.
...
описывает, на что значения могут ссылаться, а второй описывает, что
...
Назовем _типизированным instruction pointer-ом_ индекс блока, рассчитывающего
...
зависимая пара из типа состояния регистров и типизированного им instruction
pointer-а.
...
instruction pointer, и определяет, как изменится стек вызовов и
типизированный типом текущего состояния регистров instruction pointer.
...
Результатом исполнения блока является совокупность результатов
...
Функция `exec-instr` не была реализована, так как к моменту ее определения
стало понятно, что приведенные выше определения обладают рядом недостатков,
которые не позволят решить поставленную задачу.
